...
def hledej():
    ...
    if len(vstup.get()) != 8:
        ...
        delete()
        text_error_label.configure(text="Chyba: Zadejte přesně 8 číslic")
        text_error_label.configure(text_color="red")
    ...
    try:
        ...
        results["ico"].configure(text=api_ares.data.get("ico", "N/A"))
        results["dic"].configure(text=api_ares.data.get("dic", "N/A"))
        results["jmeno"].configure(text=api_ares.data.get("obchodniJmeno", "N/A"))
        ...
        results["kod_stat"].configure(text=sidlo_data.get("kodStatu", "N/A"))
        results["zalozeno"].configure(text=api_ares.data.get("datumVzniku", "N/A"))
        ...
        results["spis_znacka"].configure(text=spisova_znacka)

        results["cznace"].configure(text=api_ares.data.get("czNace", "N/A"))
        results["aktualizace"].configure(text=api_ares.data.get("datumAktualizace", "N/A"))

    except requests.exceptions.ConnectionError:
        # Konkrétní chyba pro problémy se sítí
        delete()
        text_error_label.configure(text="Chyba připojení.")
        text_error_label.configure(text_color="orange")

    except requests.exceptions.HTTPError as e:
        # Zde zachytíme specifickou chybu 404
        if e.response.status_code == 404:
            delete()
            text_error_label.configure(text="Zadané IČO nebylo nalezeno.")
            text_error_label.configure(text_color="orange")
        else:
...
nadpis_frame = customtkinter.CTkFrame(window)
...
buton_frame = customtkinter.CTkFrame(window)
...
vstup = customtkinter.CTkEntry(input_frame,
              font=FONT_NADPIS,
...
tlacitko = customtkinter.CTkButton(input_frame, text="Hledej", command=hledej, width=200, font=FONT_BUTTONS)
...
for row, (text, key) in enumerate(labels):
    ...
    lbl = customtkinter.CTkLabel(result_frame, text=text, font=FONT_TEXT)
    lbl.grid(row=row, column=0, sticky="e", padx=10)

    # Label pro zobrazení výsledků
    results[key] = customtkinter.CTkLabel(result_frame, text="", font=FONT_TEXT)
    results[key].grid(row=row, column=1, sticky="w")

# Label pro zobrazení chybových hlášek
text_error_label = customtkinter.CTkLabel(result_frame, text="", text_color="red", font=FONT_NADPIS)
