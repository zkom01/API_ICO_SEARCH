...
def hledej():
    ...
    if vstup.get():
        try:
            ...
            # Tento globální except blok zachytí případné chyby s připojením k API nebo neplatnou odpovědí
            text_error_label["text"] = f"Chyba při dotazu na API: {e}"
...
for row, (text, key) in enumerate(labels):
    ...
    results[key] = Label(result_frame, text=".", bg=BACKGROUND_COLOR, fg=TEXT_COLOR, font=FONT_TEXT)
